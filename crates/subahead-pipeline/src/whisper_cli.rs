...
use subahead_core::{TranscriptSegment, TranscriptionRequest, TranscriptionResult};
...
pub fn transcribe_with_whisper_cli(
    ...
    let model_path = request
        ...
        .ok_or_else(|| {
            PipelineError::AsrUnavailable(
                "no Whisper model found; expected ggml-base.bin in the SubAhead model directory"
                    .into(),
            )
        })?;
    ...
    let output_dir = env::temp_dir().join("subahead").join("transcriptions");
