...
1. Fingerprint the local file and build a pipeline key from the selected audio stream, Whisper runtime/model hash, language mode, VAD model, chunk size, extraction context, and cue-segmentation version. Only exactly matching checkpoints are restored.
...
6. Persist each source window atomically, replacing stale overlapping cues and marking the checkpoint complete in the same SQLite transaction.
7. Queue cloud translation independently from ASR. Translation processes bounded batches with prior dialogue context, while source transcription continues ahead of playback.
8. Persist source and provider/target-specific translations separately. A source edit invalidates every stale provider translation for that cue.
9. Publish a full initial snapshot followed by ordered incremental cue patches over Tauri Channels.
10. Render only cached cues against a 100 ms native player clock using binary-search lookup; neither the WebView render loop nor libVLC calls ASR or translation.
...
On Windows, Myna Player creates an application-owned child HWND below WebView2 and binds libVLC to that surface. Verified libVLC DLLs/plugins and statically built Whisper, FFmpeg, and FFprobe sidecars are staged into the Tauri bundle by the Windows packaging scripts.
...
- A strict Content Security Policy, disabled asset protocol, and narrow Tauri capabilities limit the WebView attack surface.
